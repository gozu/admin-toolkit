...
![Version](https://img.shields.io/badge/version-0.4.839-blue)
...
**Container Execs** *(tool)* streams the live container-execution inventory (K8s workloads per project, recipe, webapp). **Docker Images** *(tool)* prunes stale images from ECR/ACR/GAR registries. **CS Templates** *(tool)* migrates code studios between templates. **Model Audit** inventories every LLM connection and model with cost and replacement hints. **K8s Insights** audits your clusters live: current server cost, estimated cost after changes, and potential savings, with plain-language explanations and expandable technical findings.

<div align="center"><img src="docs/screenshots/k8s-insights.png" alt="K8s Insights — current server cost, estimated cost after changes, and where savings come from" width="850" /></div>
