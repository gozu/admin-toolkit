...
test.describe('Cobuild immediate reads', () => {
    ...
    test(`data is visible while interpretation waits, then ${outcome}`, async ({ page }) => {
      ...
      await page.route('**/api/agents/chat', (route) => route.fulfill({
        contentType: 'text/event-stream', body: sse('agent_event', {
          eventKind: 'read_interpretation', eventData: { name: 'toolkit_get', result: {
            version: '1', runningVersion: '1', backendStale: false,
            executionRoute: { host: 'local', variant: 'compact-read', submittedAt: Date.now() / 1000,
              returnSeconds: 0.75, interpretation: { status: 'pending', turnId: 'read-a', viewTicket: 'read-only-ticket' } },
          } },
        }) + sse('done', { durationMs: 800 }),
      }));
      await enterAgentsPage(page);
      const composer = page.getByPlaceholder(/Message the agent/);
      await composer.fill('Read the toolkit version');
      await composer.press('Enter');
      await expect(page.getByText('toolkit_get · Data ready')).toBeVisible();
      await expect(page.getByText('Cobuild interpreting…')).toBeVisible();
